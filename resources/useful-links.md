# Useful Links & Resources

A curated list of resources for Terraform Associate 004 exam preparation.

---

## 📚 Official Documentation

### HashiCorp Documentation
- [Terraform Documentation](https://developer.hashicorp.com/terraform/docs) - Official docs
- [Terraform Language Documentation](https://developer.hashicorp.com/terraform/language) - HCL reference
- [Terraform CLI Documentation](https://developer.hashicorp.com/terraform/cli) - Commands reference
- [Terraform Providers](https://registry.terraform.io/browse/providers) - Provider registry

### Exam-Specific Resources
- [Terraform Associate 003 Tutorial](https://developer.hashicorp.com/terraform/tutorials/certification-003) - HashiCorp's official prep
- [Exam Study Guide](https://developer.hashicorp.com/terraform/tutorials/certification-003/associate-study-003) - Detailed study guide
- [Sample Questions](https://developer.hashicorp.com/terraform/tutorials/certification-003/associate-questions) - Official practice questions

---

## 🎓 Learning Platforms

### Free Resources
- [HashiCorp Learn](https://developer.hashicorp.com/terraform/tutorials) - Free tutorials
- [Terraform on AWS](https://developer.hashicorp.com/terraform/tutorials/aws-get-started) - AWS-specific tutorials
- [Terraform on Azure](https://developer.hashicorp.com/terraform/tutorials/azure-get-started) - Azure-specific tutorials
- [YouTube - Terraform Tutorials](https://www.youtube.com/hashicorp) - Official HashiCorp channel

### Paid Resources
- **A Cloud Guru** - Terraform courses
- **Linux Academy / Cloud Academy** - Certification prep
- **Udemy** - Various Terraform courses (look for highly-rated ones)
- **Pluralsight** - Terraform paths

---

## 📝 Practice Exams

### Simulados Recomendados
- **🎯 Udemy - Bryan Krausen (Principal)**  
  [HashiCorp Certified Terraform Associate 004 - Practice Exams](https://www.udemy.com/home/my-courses/learning/#:~:text=HashiCorp%20Certified%20Terraform%20Associate%20004%20%2D%20Practice%20Exams)  
  ⭐ Simulados escolhidos para este estudo - mesmo autor do curso de preparação

- **ExamPro** - Terraform Associate course
- **Whizlabs** - Terraform Associate 003/004 Practice Exams (alternativa)

### Free Practice Questions
- [Terraform Associate Sample Questions](https://developer.hashicorp.com/terraform/tutorials/certification-003/associate-questions)
- [Exam Topics - Terraform](https://www.examtopics.com/exams/hashicorp/terraform-associate/)

---

## 🔧 Tools & Utilities

### VS Code Extensions
- **HashiCorp Terraform** - Official extension with syntax highlighting
- **Terraform Autocomplete** - Auto-completion support
- **Terraform Docs** - Documentation viewer

### CLI Tools
```bash
# tfenv - Terraform version manager
git clone https://github.com/tfutils/tfenv.git ~/.tfenv

# terraform-docs - Generate docs from modules
git clone https://github.com/terraform-docs/terraform-docs

# tflint - Terraform linter
git clone https://github.com/terraform-linters/tflint

# checkov - Security scanner
pip install checkov
```

---

## 🌐 Community Resources

### Forums & Communities
- [HashiCorp Discuss](https://discuss.hashicorp.com/c/terraform/15) - Official forum
- [Reddit - r/Terraform](https://www.reddit.com/r/Terraform/) - Community discussions
- [Stack Overflow - Terraform](https://stackoverflow.com/questions/tagged/terraform) - Q&A
- [Terraform Community Slack](https://terraform-community.slack.com/) - Real-time chat

### GitHub Repositories
- [Awesome Terraform](https://github.com/shuaibiyy/awesome-terraform) - Curated list
- [Terraform Best Practices](https://github.com/antonbabenko/terraform-best-practices) - Best practices guide
- [Terraform AWS Modules](https://github.com/terraform-aws-modules) - Official AWS modules

---

## 🏗️ Lab Environments

### Free Tier Options
- **AWS Free Tier** - 12 months free, always free services
- **Azure Free Account** - $200 credit, 12 months free services
- **GCP Free Tier** - $300 credit, always free services
- **Oracle Cloud Free Tier** - Always free resources

### Sandbox Environments
- [Katacoda Terraform Scenarios](https://www.katacoda.com/courses/terraform) - Interactive labs
- [Play with Terraform](https://labs.play-with-docker.com/) - Docker-based (limited)
- [CloudAcademy Labs](https://cloudacademy.com/lab/) - Various cloud labs

---

## 📖 Books & eBooks

- **Terraform: Up & Running** by Yevgeniy Brikman - Comprehensive guide
- **The Terraform Book** by James Turnbull - Beginner to intermediate
- **Infrastructure as Code** by Kief Morris - General IaC concepts

---

## 🎯 Exam Tips & Strategies

### Study Strategy
1. Read all 9 objectives thoroughly
2. Complete hands-on labs
3. Take practice exams
4. Review wrong answers
5. Focus on weak areas

### Exam Day Tips
- Get good sleep before exam
- Test your setup beforehand
- Have ID ready
- Read questions carefully
- Flag uncertain questions for review

---

## 🔗 Quick Reference Links

### Documentation Quick Links
| Topic | Link |
|-------|------|
| Providers | https://registry.terraform.io/browse/providers |
| Modules | https://registry.terraform.io/browse/modules |
| Functions | https://developer.hashicorp.com/terraform/language/functions |
| CLI | https://developer.hashicorp.com/terraform/cli/commands |
| State | https://developer.hashicorp.com/terraform/language/state |
| Backends | https://developer.hashicorp.com/terraform/language/settings/backends |

### Cheat Sheets
- [Terraform CLI Cheat Sheet](https://developer.hashicorp.com/terraform/cli/commands)
- [HCL Syntax](https://developer.hashicorp.com/terraform/language/syntax/configuration)
- [Built-in Functions](https://developer.hashicorp.com/terraform/language/functions)

---

## 📅 Certification Maintenance

- Certification valid for **2 years**
- Renewal through re-examination
- Stay updated with new Terraform versions
- Follow [Terraform Announcements](https://discuss.hashicorp.com/c/terraform/announcements/10)

---

**Last Updated:** 2024

**Contributing:** Feel free to suggest additional resources via PR!
